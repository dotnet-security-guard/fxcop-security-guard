            ...
            foreach(FxCopIssue issue in issues) {
                if (issue.issueType.Equals(type) &&
                    ...
                    issue.line.Equals(line)) {
                        Console.WriteLine("Successful assertion : {0} in {1}:{2}", type, file, line);
                        return;
                }
                if(issue.file.Equals(file)) {
                    Console.WriteLine("Issue : <{0}> matching the file {1}",issue.ToString(),file);
                }
            }

            Assert.Fail(String.Format("The issue {0} was not found in {1}:{2}",type,file,line));
        }
